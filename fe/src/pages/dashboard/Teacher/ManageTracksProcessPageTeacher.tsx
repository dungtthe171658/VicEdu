...
const ManageTracksProcessPageTeacher: React.FC = () => {
  ...
  const fetchStudentProgress = async () => {
    try {
      ...
      const courseIds = courses.map((c: any) => c._id).filter(Boolean);
      ...
      const enrollmentsRes = await enrollmentApi.getByCoursesForTeacher(courseIds);
      ...
      try {
        const attemptsRes = await quizApi.getAttemptsByCoursesForTeacher(courseIds);
        quizAttempts = Array.isArray(attemptsRes?.data)
          ? attemptsRes.data
      ...
      enrollments.forEach((enrollment: any) => {
        ...
        if (!studentData) {
          // Get user info from enrollment
          const userInfo = enrollment.user_id;
          if (!userInfo) return;
